...
echo "\n🔍 DETAILED ANALYSIS: search/index.blade.php\n";
echo "==========================================================\n\n";

// Đọc file search/index.blade.php
...
foreach (glob('resources/lang/en/*.php') as $file) {
    $key = basename($file, '.php');
    $translation_files['en'][$key] = include $file;
}
...
function resolveTranslationKey($key, $translation_files) {
    ...
    if (strpos($key, '.') !== false) {
        ...
        // Key đơn, tìm trong common
        if (isset($translation_files['vi']['common'][$key])) {
            return $translation_files['vi']['common'][$key];
        }
    }
