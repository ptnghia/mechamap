...
// Script to update Bootstrap Icons to Feather Icons in admin views
...
$iconMapping = [
    ...
    'bi bi-chat-left-text' => 'data-feather="message-square"',
    'bi bi-chat-right-text' => 'data-feather="message-circle"',
    ...
    'bi bi-box-arrow-right' => 'data-feather="log-out"',
    'bi bi-box-arrow-up-right' => 'data-feather="external-link"',
    'bi bi-key' => 'data-feather="key"',
    'bi bi-shield' => 'data-feather="shield"',
    'bi bi-lock' => 'data-feather="lock"',
    'bi bi-unlock' => 'data-feather="unlock"',
    'bi bi-refresh' => 'data-feather="refresh-cw"',
    'bi bi-power' => 'data-feather="power"',
];

function updateIconsInFile($filePath, $iconMapping) {
    ...
    foreach ($iconMapping as $oldIcon => $newIcon) {
        // Replace class="bi bi-xxx" with data-feather="xxx"
        $content = preg_replace(
            '/class="' . preg_quote($oldIcon, '/') . '"/',
            $newIcon,
            $content
        );
        
        // Also handle cases where there are additional classes
        $content = preg_replace(
            '/class="([^"]*?)' . preg_quote($oldIcon, '/') . '([^"]*?)"/',
            'class="$1" ' . $newIcon,
            $content
...
// List of important admin view files to update
$filesToUpdate = [
...
foreach ($filesToUpdate as $file) {
    updateIconsInFile($file, $iconMapping);
}

echo "\nIcon update process completed!\n";
echo "Note: Some icons may need manual adjustment for proper styling.\n";
echo "Remember to add style='width: 16px; height: 16px;' for inline icons if needed.\n";
?>
