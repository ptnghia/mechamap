                ...
                <div class="card-body">
                    <form action="{{ route('threads.index') }}" method="GET" class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select" id="forum" name="forum">
                                ...
                                @foreach($forums as $forum)
                                    <option value="{{ $forum->id }}" {{ request('forum') == $forum->id ? 'selected' : '' }}>{{ $forum->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-8">
                            ...
                            <input type="text" class="form-control" id="search" name="search" value="{{ request('search') }}" placeholder="Search threads...">
                        </div>
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        @forelse($topContributors as $user)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <span class="badge bg-primary rounded-pill">{{ $user->threads_count + $user->comments_count }} posts</span>
                            </li>
                        @empty
            ...
            <div class="card mb-4">
                ...
                <div class="card-body p-0">
                    <ul class="list-group list-group-flush">
                        @foreach($categories as $category)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <a href="{{ route('threads.index', ['category' => $category->id]) }}" class="text-decoration-none">{{ $category->name }}</a>
            ...
            <div class="card">
                ...
                <div class="card-body p-0">
                    <ul class="list-group list-group-flush">
                        @foreach($forums as $forum)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <a href="{{ route('threads.index', ['forum' => $forum->id]) }}" class="text-decoration-none">{{ $forum->name }}</a>
