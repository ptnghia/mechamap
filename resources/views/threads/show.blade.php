...
<div class="container">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            @if($thread->forum)
            <li class="breadcrumb-item"><a href="{{ route('threads.index', ['forum' => $thread->forum->id]) }}">{{
                    $thread->forum->name }}</a></li>
            @endif
            @if($thread->category)
            <li class="breadcrumb-item"><a href="{{ route('threads.index', ['category' => $thread->category->id]) }}">{{
                    $thread->category->name }}</a></li>
            @endif
    ...
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <a href="{{ route('profile.show', $thread->user) }}" class="fw-bold text-decoration-none">{{
                        $thread->user->name }}</a>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        <a href="{{ asset('storage/' . $media->path) }}" data-lightbox="thread-images">
                            <img src="{{ asset('storage/' . $media->path) }}" alt="Thread image"
        ...
        <div class="card-footer d-flex justify-content-between">
            <div>
                ...
                <form action="{{ route('threads.like', $thread) }}" method="POST" class="d-inline">
                    @csrf
                    <button type="submit"
                        class="btn btn-sm {{ Auth::check() && $isLiked ? 'btn-primary' : 'btn-outline-primary' }}">
                ...
                <form action="{{ route('threads.save', $thread) }}" method="POST" class="d-inline ms-2">
                    @csrf
                    <button type="submit"
                        class="btn btn-sm {{ Auth::check() && $isSaved ? 'btn-success' : 'btn-outline-success' }}">
            ...
            <div>
                ...
                <div class="dropdown d-inline">
                    <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" id="shareDropdown"
                    ...
                    <ul class="dropdown-menu" aria-labelledby="shareDropdown">
                        ...
                        <li><a class="dropdown-item" href="#"
                                onclick="navigator.clipboard.writeText('{{ request()->url() }}'); alert('Link copied to clipboard!'); return false;"><i
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div class="btn-group">
                <a href="{{ request()->fullUrlWithQuery(['sort' => 'oldest']) }}"
                    class="btn btn-sm {{ request('sort', 'oldest') == 'oldest' ? 'btn-primary' : 'btn-outline-primary' }}">Oldest
                    first</a>
                <a href="{{ request()->fullUrlWithQuery(['sort' => 'newest']) }}"
                    class="btn btn-sm {{ request('sort') == 'newest' ? 'btn-primary' : 'btn-outline-primary' }}">Newest
                    first</a>
                <a href="{{ request()->fullUrlWithQuery(['sort' => 'reactions']) }}"
                    class="btn btn-sm {{ request('sort') == 'reactions' ? 'btn-primary' : 'btn-outline-primary' }}">Most
                    reactions</a>
            </div>
        </div>
                ...
                <div class="comment-attachments mt-3">
                    <div class="row g-2">
                        ...
                        <div class="col-md-3 col-sm-4 col-6">
                            <a href="{{ asset('storage/' . $attachment->file_path) }}" class="d-block"
                                data-lightbox="comment-{{ $comment->id }}-images"
                                data-title="{{ $attachment->file_name }}">
                                <img src="{{ asset('storage/' . $attachment->file_path) }}"
                                    alt="{{ $attachment->file_name }}" class="img-fluid rounded">
                            </a>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="editor-container">
                        <div class="editor-toolbar">
                            <button type="button" class="btn btn-sm" onclick="formatText('bold')"><i
                                    class="bi bi-type-bold"></i></button>
                            <button type="button" class="btn btn-sm" onclick="formatText('italic')"><i
                                    class="bi bi-type-italic"></i></button>
                            <button type="button" class="btn btn-sm" onclick="formatText('underline')"><i
                                    class="bi bi-type-underline"></i></button>
                            <button type="button" class="btn btn-sm" onclick="formatText('strikethrough')"><i
                                    class="bi bi-type-strikethrough"></i></button>
                            <div class="vr mx-1"></div>
                            <button type="button" class="btn btn-sm" onclick="formatText('link')"><i
                                    class="bi bi-link"></i></button>
                            <button type="button" class="btn btn-sm" id="image-upload-btn"><i
                                    class="bi bi-image"></i></button>
                            <div class="vr mx-1"></div>
                            <button type="button" class="btn btn-sm" onclick="formatText('quote')"><i
                                    class="bi bi-chat-quote"></i></button>
                            <button type="button" class="btn btn-sm" onclick="formatText('code')"><i
                                    class="bi bi-code"></i></button>
                            <div class="vr mx-1"></div>
                            <button type="button" class="btn btn-sm" onclick="formatText('ul')"><i
                                    class="bi bi-list-ul"></i></button>
                            <button type="button" class="btn btn-sm" onclick="formatText('ol')"><i
    ...
    @else
    <div class="alert alert-info">
        Please <a href="{{ route('login') }}">login</a> or <a href="{{ route('register') }}">register</a> to post a
        reply.
    </div>
    @endauth
    ...
    <div class="related-threads mt-4">
        ...
        <div class="list-group">
            ...
            <a href="{{ route('threads.show', $relatedThread) }}" class="list-group-item list-group-item-action">
                ...
                <small>By {{ $relatedThread->user->name }} · {{ $relatedThread->view_count }} views · {{
                    $relatedThread->allComments->count() }} replies</small>
            </a>
            @endforeach
        </div>
    </div>
