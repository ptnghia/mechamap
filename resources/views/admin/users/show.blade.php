        ...
        <div class="col-md-4 mb-4">
                ...
                <div class="card-body text-center">
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <a href="{{ route('admin.users.roles', $user) }}" class="btn btn-sm btn-primary">
                            <i class="fas fa-users-cog me-1"></i> {{ 'Quản lý Multiple Roles' }}
                        </a>
                        @endadminCan
                    </div>
                </div>
            ...
            <div class="card mt-4">
                ...
                <div class="card-body">
                    ...
                    @if($user->banned_at)
                        <div class="mb-0">
                            <label class="form-label text-muted">{{ 'Thông tin cấm' }}</label>
                            <div class="alert alert-danger mb-0">
                                <p class="mb-1"><strong>{{ 'Thời gian:' }}</strong> {{ $user->banned_at->format('d/m/Y H:i:s') }}</p>
                                <p class="mb-0"><strong>{{ 'Lý do:' }}</strong> {{ $user->banned_reason }}</p>
                            </div>
                        </div>
                    @endif
                </div>
            ...
            <div class="card mt-4">
                <div class="card-header">
                    <h5 class="card-title mb-0">{{ 'Đặt lại mật khẩu' }}</h5>
                </div>
                <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6 mb-4">
                    <div class="card h-100">
                        <div class="card-header">
                            <h5 class="card-title mb-0">{{ 'Chủ đề gần đây' }}</h5>
                        </div>
                            ...
                            <div class="list-group list-group-flush">
                                @forelse($stats['latest_threads'] as $thread)
                                    <a href="{{ route('forums.show', $thread->forum) }}#thread-{{ $thread->id }}" class="list-group-item list-group-item-action" target="_blank">
                                        ...
                                        <small class="text-muted">
                                            <i class="fas fa-comment"></i> {{ $thread->forum ? $thread->forum->name : 'Unknown Forum' }}
                                        </small>
                                    </a>
                                @empty
                                    <div class="list-group-item text-center py-4">
                                        <i class="fas fa-comment text-muted fs-1 d-block mb-2"></i>
                                        <p class="text-muted mb-0">{{ 'Chưa có chủ đề nào' }}</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                        @if(count($stats['latest_threads']) > 0)
                            <div class="card-footer text-end">
                                <a href="#" class="btn btn-sm btn-outline-primary">{{ 'Xem tất cả chủ đề' }}</a>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <form action="{{ route('admin.users.destroy', $user) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="alert alert-danger">
                            ...
                            {{ 'Cảnh báo: Hành động này không thể hoàn tác. Tất cả dữ liệu liên quan đến thành viên này sẽ bị xóa vĩnh viễn.' }}
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ 'Hủy' }}</button>
