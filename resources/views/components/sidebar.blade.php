...
<div class="sidebar-container">
    ...
    <div class="card shadow-sm rounded-3 mb-4">
        <div class="card-header">
            <h5 class="card-title mb-0"><i class="bi bi-info-circle-fill me-2 text-primary"></i>{{ config('app.name') }}</h5>
        </div>
        <div class="card-body">
            <div class="forum-stats mb-3">
                ...
                <div class="text-muted small mb-3">
                    <i class="bi bi-calendar-check me-1"></i> Hoạt động từ {{ \Carbon\Carbon::parse(config('app.established_year', '2023'))->format('Y') }}
                </div>
            </div>

            <p class="mb-3">{{ config('app.description', 'Cộng đồng chia sẻ thông tin về kiến trúc, xây dựng, giao thông, quy hoạch đô thị và nhiều chủ đề khác.') }}</p>
                    ...
                    <a href="{{ route('threads.show', $thread) }}" class="list-group-item list-group-item-action py-2">
                        <div class="d-flex">
                            <div class="flex-shrink-0 me-2">
                                <img src="{{ get_avatar_url($thread->user) }}" alt="{{ $thread->user->name }}" class="rounded-circle" width="32" height="32">
                            </div>
                            <div>
                                <h6 class="mb-1 text-truncate" style="max-width: 100%;">{{ $thread->title }}</h6>
                                <div class="d-flex align-items-center small text-muted">
                                    <span class="me-2"><i class="bi bi-person-fill me-1"></i>{{ $thread->user->name }}</span>
                                    <span class="me-2"><i class="bi bi-chat-dots me-1"></i>{{ $thread->comments->count() }}</span>
                                </div>
                            </div>
                        </div>
                    </a>
        ...
        <div class="card-body p-0">
            <div class="list-group list-group-flush">
                ...
                @forelse($topContributors as $user)
                    <a href="{{ route('profile.show', $user->username) }}" class="list-group-item list-group-item-action py-2">
                        <div class="d-flex align-items-center">
                            <div class="flex-shrink-0 me-2">
                                <img src="{{ get_avatar_url($user) }}" alt="{{ $user->name }}" class="rounded-circle" width="32" height="32">
                            </div>
                            <div>
                                <h6 class="mb-1">{{ $user->name }}</h6>
                                <p class="text-muted small mb-0">{{ $user->threads_count + $user->comments_count }} đóng góp</p>
                            </div>
                        </div>
                    </a>
                @empty
        ...
        <div class="card-body">
            ...
            @foreach($relatedCommunities as $community)
                <div class="d-flex align-items-center mb-2">
                    <div class="flex-shrink-0 me-2">
                        <img src="{{ get_image_url($community['image']) }}" alt="{{ $community['name'] }}" class="rounded shadow-sm" width="40" height="40">
                    </div>
                    <div>
                        <h6 class="mb-1">
                            <a href="{{ $community['url'] }}" class="text-decoration-none">{{ $community['name'] }}</a>
                        </h6>
                        <p class="text-muted small mb-0"><i class="bi bi-people-fill me-1"></i>{{ $community['members'] }} thành viên</p>
                    </div>
                </div>
            @endforeach
        </div>
