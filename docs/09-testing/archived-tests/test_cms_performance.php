...
try {
    ...
    $filteredTemplates = ContentTemplate::where('template_type', 'calculation_guide')
        ...
        ->whereIn('industry_sector', ['automotive', 'manufacturing'])
    ...
    $analytics = ContentTemplate::selectRaw('template_type, COUNT(*) as count, AVG(usage_count) as avg_usage')
        ->where('is_active', true)
    ...
    $start = microtime(true);
    $engineeringTemplates = ContentTemplate::where('industry_sector', 'like', '%automotive%')
    ...
    $mfgTemplates = ContentTemplate::where('template_type', 'manufacturing_process')
        ->orWhere('industry_sector', 'manufacturing')
        ->get();
