...
@section('title', 'Tạo diễn đàn mới')

@section('page-title')
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font-size-18">Tạo diễn đàn mới</h4>
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="javascript: void(0);">MechaMap</a></li>
                    <li class="breadcrumb-item active">Tạo diễn đàn mới</li>
                </ol>
            </div>
        </div>
    </div>
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="card-title mb-0">{{ __('Tạo diễn đàn mới') }}</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.forums.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('parent_id') is-invalid @enderror" id="parent_id" name="parent_id">
                        ...
                        @foreach($forums as $forum)
                            <option value="{{ $forum->id }}" {{ old('parent_id') == $forum->id ? 'selected' : '' }}>{{ $forum->name }}</option>
                        @endforeach
                    </select>
                    @error('parent_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="is_private" name="is_private" {{ old('is_private') ? 'checked' : '' }}>
