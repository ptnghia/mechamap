...
class UpdateDigitalProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // Digital Files (Optional for update)
            'digital_files' => 'nullable|array|max:10',
            'digital_files.*' => [
                'required',
                'file',
            ...
            // Remove existing digital files
            'remove_digital_files' => 'nullable|string',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Check if product will have digital files after update
            $hasExistingFiles = $product && !empty($product->digital_files);
            $hasNewFiles = $this->hasFile('digital_files');
            $removingFiles = $this->filled('remove_digital_files');
            
            if ($removingFiles) {
                $removeIndices = array_map('trim', explode(',', $this->input('remove_digital_files')));
                $existingFiles = $product->digital_files ?? [];
                $remainingFiles = count($existingFiles) - count($removeIndices);
                
                if ($remainingFiles <= 0 && !$hasNewFiles) {
                    $validator->errors()->add('digital_files', 'Sản phẩm digital phải có ít nhất một file.');
                }
            } elseif (!$hasExistingFiles && !$hasNewFiles) {
                $validator->errors()->add('digital_files', 'Sản phẩm digital phải có ít nhất một file.');
            }
            
            // Validate total file size for new files
            if ($this->hasFile('digital_files')) {
                ...
                // Max total size for new files: 500MB
                if ($totalSize > 524288000) {
                    $validator->errors()->add('digital_files', 'Tổng dung lượng các file digital mới không được vượt quá 500MB.');
                }
            }
