    ...
    <div class="row">
        <div class="col-md-3 mb-4">
            ...
            <div class="card mt-4">
                ...
                <div class="card-body">
                    <p class="card-text">
                        {{ __('Cấu hình Social Media giúp tối ưu hóa cách trang web của bạn hiển thị khi được chia sẻ trên các mạng xã hội.') }}
                    </p>
                    <p class="mb-0">
                        {{ __('Các thẻ Open Graph (og:) được sử dụng bởi Facebook và nhiều nền tảng khác, trong khi các thẻ Twitter Card được sử dụng bởi Twitter.') }}
                    </p>
                </div>
            </div>
        ...
        <div class="col-md-9">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.seo.update-social') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <label for="twitter_card" class="form-label">{{ __('Loại Twitter Card') }}</label>
                            <select class="form-select @error('twitter_card') is-invalid @enderror" id="twitter_card" name="twitter_card">
                                <option value="summary" {{ (old('twitter_card', $settings['twitter_card'] ?? '')) == 'summary' ? 'selected' : '' }}>{{ __('Summary') }}</option>
                                <option value="summary_large_image" {{ (old('twitter_card', $settings['twitter_card'] ?? '')) == 'summary_large_image' ? 'selected' : '' }}>{{ __('Summary with Large Image') }}</option>
                                <option value="app" {{ (old('twitter_card', $settings['twitter_card'] ?? '')) == 'app' ? 'selected' : '' }}>{{ __('App') }}</option>
                                <option value="player" {{ (old('twitter_card', $settings['twitter_card'] ?? '')) == 'player' ? 'selected' : '' }}>{{ __('Player') }}</option>
                            </select>
                            <div class="form-text">{{ __('Loại thẻ Twitter Card để sử dụng khi chia sẻ trang web của bạn trên Twitter.') }}</div>
                            @error('twitter_card')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="card mt-4">
                <div class="card-header">
                    <h5 class="card-title mb-0">{{ __('Kiểm tra công cụ chia sẻ xã hội') }}</h5>
                </div>
                <div class="card-body">
                    <div class="row">
